...
{   'PreInput': None,
    ...
    'promptB': '',
    'system_prompt': None,
    'terminate_response': ['<|prompt|>', '<|answer|>', '<|endoftext|>']}
...
if __name__ == "__main__":
    ...
    for model in models_list:
        print(model + "\n")
        conv = get_conv_template(model)
        conv.append_message(conv.roles[0], "I'm User Hi!")
        conv.append_message(conv.roles[1], "Hey dumb!")
        conv.append_message(conv.roles[0], "sing me a song?")
        conv.append_message(conv.roles[1], None)
        print(conv.get_prompt())
        print('\n\n')
        
    
    htmlAnswer = "<br> asdfasd fasldkfajsldf sdfsdf }<br>"
    print(htmlAnswer[-4:])
    print(htmlAnswer[4:])
    print(htmlAnswer[:4])
    print(htmlAnswer[:-4])
    ...
    print(htmlAnswer[:-1])
    if htmlAnswer[-4:] == "<br>":
        htmlAnswer = htmlAnswer[:-4]
    if htmlAnswer[:4] == "<br>":
        htmlAnswer = htmlAnswer[4:]
    if htmlAnswer[-1] == "}":
        htmlAnswer = htmlAnswer[:-1]
